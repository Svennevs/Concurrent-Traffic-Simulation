...
void Intersection::addVehicleToQueue(std::shared_ptr<Vehicle> vehicle)
{
    ...
    // FP.6b : use the methods TrafficLight::getCurrentPhase and TrafficLight::waitForGreen to block the execution until the traffic light turns green.
  	...
  	// modified: getCurrentPhase not used (would result in data race), TrafficLight::waitForGreen only returns when green and does so immediately if it 		is.
	_trafficLight.waitForGreen();
     
  	 lck.lock();
     std::cout << "Intersection #" << _id << ": Vehicle #" << vehicle->getID() << " is granted entry and has green light" << std::endl;
     lck.unlock();
}
